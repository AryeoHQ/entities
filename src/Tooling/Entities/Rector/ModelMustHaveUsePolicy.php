...
final class ModelMustHaveUsePolicy extends Rule
{
    ...
    public function handle(Node $node): null|Node
    {
        ...
        $policyFqcn = $this->derivePolicyFqcn($node);

        $attribute = new Attribute(
            new FullyQualified(UsePolicy::class),
            [new Arg(new ClassConstFetch(new FullyQualified($policyFqcn), 'class'))],
        );
    ...
    private function derivePolicyFqcn(Class_ $node): string
    {
        $namespace = $node->namespacedName?->slice(0, -1)?->toString() ?? '';

        return $namespace.'\\Policy\\Policy';
    }
}
