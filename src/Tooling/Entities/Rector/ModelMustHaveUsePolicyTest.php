...
class ModelMustHaveUsePolicyTest extends TestCase
{
    use GetsFixtures;
    use ParsesNodes;
    ...
    #[Test]
    public function it_adds_use_policy_attribute_when_missing(): void
    {
        $classNode = $this->getClassNode($this->getFixturePath('Entities/ModelWithoutUsePolicy.php'));

        $this->assertFalse($this->hasAttribute($classNode, UsePolicy::class));

        $rule = $this->resolveRule(ModelMustHaveUsePolicy::class);
        $result = $rule->refactor($classNode);

        $this->assertInstanceOf(Class_::class, $result);
        $this->assertTrue($this->hasAttribute($result, UsePolicy::class));
    }

    #[Test]
    public function it_does_not_modify_when_use_policy_already_present(): void
    {
